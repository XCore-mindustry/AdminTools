...
public class UIController {
    ...
    private void buildPortalContent(Table body) {
        ...
        serverCountLabel = topBar.add("Поиск серверов...").color(LucidTheme.textDim).left().growX().get();
        serverCountLabel.setFontScale(0.85f);

        TextButton refreshBtn = new TextButton(Iconc.refresh + " Обновить", Styles.cleart);
    ...
    private void refreshServers() {
        ...
        onlineServers.clear();
        rebuildServerList();
        if (serverCountLabel != null) serverCountLabel.setText("Опрос серверов...");
            ...
            Vars.net.pingHost(IP, currentPort,
                host -> {
                    ...
                    if (serverCountLabel != null) {
                        serverCountLabel.setText("Онлайн: " + onlineServers.size + " серв.");
                    }
                    rebuildServerList();
                },
                exception -> {
    ...
    private void rebuildServerList() {
        ...
        if (onlineServers.isEmpty()) {
            serverListTable.add("[lightgray]Поиск серверов XCore...[]").pad(24f).center().row();
            return;
        }
        ...
        for (int i = 0; i < onlineServers.size; i++) {
            ...
            if (s.players > 0) {
                Label playersLbl = row.add(s.players + " игр.").color(LucidTheme.textDim).padRight(8f).get();
